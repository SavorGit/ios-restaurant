...
- (void)createUI
{
    ...
    [self.chooseButton setTitle:@"添加至视频列表" forState:UIControlStateNormal];
...
- (void)addPhotoToLibrary
{
    ...
    if (assetIds.count > 0) {
        if (self.sliderModel) {
            ...
            RDTextAlertView * alert = [[RDTextAlertView alloc] initWithMaxNumber:20 message:@"输入视频列表名称，最多20个字"];
            ...
            RDAlertAction * action2 = [[RDAlertAction alloc] initWithTitle:@"确定" handler:^{
                ...
                if (weakAlert.textView.text.length == 0) {
                    [Helper showTextHUDwithTitle:@"视频列表名称不能为空" delay:1.f];
                    return;
                }
...
- (void)updateChooseStatus
{
    if (self.selectArray.count == 0) {
        ...
        if (self.sliderModel) {
            [self.chooseButton setTitle:[NSString stringWithFormat:@"添加至视频列表 '%@'", self.sliderModel.title] forState:UIControlStateNormal];
        }else{
            [self.chooseButton setTitle:@"创建视频列表" forState:UIControlStateNormal];
        }
